...
- **2026-08-27 后台推送失效根因（iOS = APNs 凭证缺失）**：实机测试（Android release APK + iOS）后台收不到系统通知。查 pg_net 日志（`net._http_response.content`）发现 iOS 推送全部返回 `InvalidCredentials`（"Could not find APNs credentials for com.richardchung.doctormeetslawyer"），**Expo 项目（projectId 148ec880-...）从未配置 APNs 推送凭证**。
  - **凭证区分（易混）**：`ios-signing/AuthKey_LSLS88W574.p8` = App Store Connect API Key（keys.txt "Key ID for App Store Connect API" = LSLS88W574），**不是** APNs Auth Key；`distribution.pem/.cer`、`certificates.p12` = Apple Distribution 签名证书，也非推送凭证。
  - APNs Auth Key 只能在 Apple Developer 后台（Certificates -> Keys -> 新建启用 Push Notifications 的 Key，记下 Key ID）创建；创建后上传到 Expo。
  - **上传完成（2026-08-27）**：key = `ios-signing/AuthKey_WADZS536MZ.p8`（Key ID `WADZS536MZ`，team `3W8574PF9N`），push key id `3fe68bb3-9fe4-4bc8-b997-c38bd5502f1f`，已绑定 `com.richardchung.doctormeetslawyer`。**上传方式（非交互）**：`eas credentials` 是纯交互命令（CreatePushKey 明示 non-interactive 不可创建）；改用 eas-cli 内部模块编程调用 GraphQL（`https://api.expo.dev/graphql`，认证头 `expo-session: <~/.expo/state.json 的 auth.sessionSecret>`）：`createPushKeyAsync` -> `createOrGetIosAppCredentialsWithCommonFieldsAsync` -> `updateIosAppCredentialsAsync({ applePushKeyId })`（模块在 `eas-cli/build/credentials/ios/api/GraphqlClient.js`）。注意 sessionSecret 是 JSON 字符串，需原样传给 `expo-session` 头。
- **Android（release APK）推送注意**：payload 必须带 `channelId: 'messages'`（对应客户端创建的高重要度渠道），否则 Expo 投递到默认渠道可能锁屏不显示/不响铃。**notify 函数已加（commit 4c8b964）**。
- **notify 函数改进（commit 4c8b964）**：① 解析 Expo 响应不再吞错误（`ok:false` + 详情透传到 pg_net 日志，可排障）；② `DeviceNotRegistered` 自动清除该用户 `push_token`（防持续发废 token）；③ payload 加 `channelId`。客户端 `src/lib/notifications.ts` 的 `setNotificationHandler` 改为前台（AppState active）抑制远程推送（`trigger.type === 'push'` 时不显示），避免与 Realtime 本地通知双横幅；本地通知与后台不受影响。
- **验证推送结果**：`SELECT id, status_code, LEFT(content, 220) AS content_preview FROM net._http_response ORDER BY id DESC LIMIT 10;`
- **Android 跨版本通知（2026-08-27）**：payload 带 `channelId: 'messages'`（API 26+ 高重要度渠道）+ `priority: 'high'`（API<26 无渠道靠 priority head-up）。权限/渠道/声音在旧版 Android 的兼容性由 expo-notifications 0.29.14 源码确认（见 android-build skill 推送章节）。notify 函数 commit `e3244e2`。
- **全链路已通（2026-08-27 实测）**：APNs key 上传后，3 个 push token 直连 Expo Push API 全部返回 `status: ok`（不再 InvalidCredentials）；notify 函数已用新代码部署；provisioning profile 含 `aps-environment: production`。iOS 后台推送已生效；Android 后台推送（release APK）走 `messages` 渠道亦已生效。
